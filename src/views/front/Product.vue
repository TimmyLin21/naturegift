  ...
  <section class="container py-5">
    <div class="row row-cols-1 row-cols-lg-2 g-6 mb-5">
      ...
      <div class="col text-start">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            <li class="breadcrumb-item">
              <router-link to='/' class="text-decoration-none link-success">Home</router-link>
            </li>
            <li class="breadcrumb-item">
              <router-link to='/products' class="text-decoration-none link-success">
        ...
        <p class="text-muted mb-4">Weight: {{product.unit}}</p>
        <p class="h3 mb-4">
          <span v-if="product.origin_price" class="text-danger text-decoration-line-through me-2">
        ...
        <div class="d-flex mb-4">
          <div class="qty me-5">
            ...
            <input class="text-center" type="number" v-model.number="qty" min="1">
            <span><BIconPlusCircle @click="qty += 1"/></span>
          </div>
          <a href="#" class="c-btn" @click.prevent="addToCart(product.id, qty)">
    ...
    <div class="row row-cols-1 row-cols-lg-2 g-6 mb-5">
      ...
      <div class="col">
        <div class="c-table">
          <h4 class="text-white py-3">Nutritional Information</h4>
          <div class="c-table__body">
            <p><span></span>Per 100g</p>
            ...
            <p class="mb-0 border-0">Fiber (g)<span>{{product.fiber}}</span></p>
          </div>
        </div>
      </div>
    </div>
    <h3 class="mb-5">Have you tried?</h3>
    <ul class="row row-cols-1 row-cols-md-2 row-cols-lg-3 list-style-none">
      <template v-for="(product, index) in products" :key="product.id">
        <li class="col mb-4" v-if="index < 3">
